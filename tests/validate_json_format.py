...
class CheckJsonFormat(object):
    ...
    def check_comments(self, text):
        ...
        content = ''.join(map(evaluate, RE_COMMENT.finditer(text)))
        return content
    ...
    def check_dangling_commas(self, text):
        ...
        def evaluate(m):
            g = m.groupdict()
            return check_comma(g, self.get_line(m.start(0))) if g["code"] is None else g["code"]

        return ''.join(map(evaluate, RE_TRAILING_COMMA.finditer(text)))
